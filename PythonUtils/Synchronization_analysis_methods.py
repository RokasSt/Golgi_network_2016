...
def Count_cells(experiment_array,no_of_cell_groups):
    
    cell_no_array=[]
    for exp_id in range(0,len(experiment_array[0])):
        if exp_specify[1][1]==True:
           for cell_group in range(0,no_of_cell_groups):
               cell_group_positions=np.loadtxt('simulations/%s/Golgi_pop%d.txt'%(experiment_array[0][exp_id],cell_group))
               dim_array=np.shape(cell_group_positions)
               cell_no_array.append(dim_array[0])
           
        else:
           for cell_group in range(0,no_of_cell_groups):
               cell_group_positions=np.loadtxt('simulations/%s/sim0/Golgi_pop%d.txt'%(experiment_array[0][exp_id],cell_group))
               dim_array=np.shape(cell_group_positions)
               cell_no_array.append(dim_array[0])
               
    return cell_no_array


def Generate_target_cell_arrays(experiment_array,specify_targets_array,no_of_cell_groups):
    indicator_of_identical_targets=True
    target_cell_array_all_experiments=[]
    x=0
    for exp_id in range(0,len(experiment_array[0])):
        experiment_parameters=[]
        experiment_parameters.append(experiment_array[0][exp_id])
        experiment_parameters.append(experiment_array[1])
        experiment_parameters.append(experiment_array[2])
       
        target_cell_array=get_cell_ids_for_sync_analysis(specify_targets_array,no_of_cell_groups,experiment_parameters)
        target_cell_array_all_experiments.append(target_cell_array)
        if exp_id >0 and x==0:
           if target_cell_array !=target_cell_array_all_experiments[0]:
              indicator_of_identical_targets=False
              x=1
    if indicator_of_identical_targets:
       target_cell_array_all_experiments=target_cell_array_all_experiments[0]
       
    return indicator_of_identical_targets, target_cell_array_all_experiments

def Raster_plot_plus_synchrony(target_equivalence_indicator,            ):
    if spike_plot_parameters[0]=="main figure":
...
def Save_sync_plot_to_separate_file():

def Save_all_trials_to_separate_files():
...
def Synchronization_analysis(sim_duration,specify_targets,no_of_groups,exp_specify,spike_plot_parameters,general_plot_parameters):
    ...
    cell_no_array=Count_cells(exp_specify,no_of_groups)
    
    n_trials=exp_specify[2]

    indicator_of_identical_targets, target_cell_array_all_experiments=Generate_target_cell_arrays(exp_specify,specify_targets,no_of_groups)
    ...
    if  indicator_of_identical_targets:
        target_cell_array=target_cell_array_all_experiments[0]
        ...
        for exp_id in range(0,len(exp_specify[0])):
            if exp_id==0:
               ...
               if spike_plot_parameters[0]=="main figure":
                  Raster_plot_plus_synchrony(indicator_of_identical_targets,n_trials,spike_plot_parameters[1],
                  trial_indicator_target=False
                  if n_trials >1:
                     target_cell_array_target_trial=target_cell_array[spike_plot_parameters[1]]
                     no_of_rasters=0
...
if __name__=="__main__":
   ...
   spike_plot_params=["main figure",3,"save all trials to one separate file","save sync plot to a separate file","pdf"]  
