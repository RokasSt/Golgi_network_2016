...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: gatesHHrates[*]/fcond, on: Component(id=Golgi_KV type=ionChannelHH), from gatesHHrates; Component(id=n type=gateHHrates)
    ? multiply applied to all instances of fcond in: <gatesHHrates> ([Component(id=n type=gateHHrates)]) c2 ([Component(id=null type=annotation), Component(id=null type=notes), Component(id=n type=gateHHrates)])
    fopenHHrates = n_fcond ? path based
    
    ? DerivedVariable is based on path: gatesHHtauInf[*]/fcond, on: Component(id=Golgi_KV type=ionChannelHH), from gatesHHtauInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHtauInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTau[*]/fcond, on: Component(id=Golgi_KV type=ionChannelHH), from gatesHHratesTau; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesTau = 1 
    
    ? DerivedVariable is based on path: gatesHHratesInf[*]/fcond, on: Component(id=Golgi_KV type=ionChannelHH), from gatesHHratesInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTauInf[*]/fcond, on: Component(id=Golgi_KV type=ionChannelHH), from gatesHHratesTauInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesTauInf = 1 
    
    ? DerivedVariable is based on path: gateHHInstantaneous[*]/fcond, on: Component(id=Golgi_KV type=ionChannelHH), from gateHHInstantaneous; null
    ? Path not present in component, using factor: 1
