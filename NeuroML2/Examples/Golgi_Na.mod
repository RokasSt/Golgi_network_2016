TITLE Mod file for component: Component(id=Golgi_Na type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Golgi_Na type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gatesHHrates[*]/fcond, on: Component(id=Golgi_Na type=ionChannelHH), from gatesHHrates; Component(id=m type=gateHHrates)
    ? multiply applied to all instances of fcond in: <gatesHHrates> ([Component(id=m type=gateHHrates), Component(id=h type=gateHHrates)]) c2 ([Component(id=null type=annotation), Component(id=null type=notes), Component(id=m type=gateHHrates), Component(id=h type=gateHHrates)])
    fopenHHrates = m_fcond * h_fcond ? path based
    
    ? DerivedVariable is based on path: gatesHHtauInf[*]/fcond, on: Component(id=Golgi_Na type=ionChannelHH), from gatesHHtauInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHtauInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTau[*]/fcond, on: Component(id=Golgi_Na type=ionChannelHH), from gatesHHratesTau; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesTau = 1 
    
    ? DerivedVariable is based on path: gatesHHratesInf[*]/fcond, on: Component(id=Golgi_Na type=ionChannelHH), from gatesHHratesInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesInf = 1 
    
    ? DerivedVariable is based on path: gatesHHratesTauInf[*]/fcond, on: Component(id=Golgi_Na type=ionChannelHH), from gatesHHratesTauInf; null
    ? Path not present in component, using factor: 1
    
    fopenHHratesTauInf = 1 
    
    ? DerivedVariable is based on path: gateHHInstantaneous[*]/fcond, on: Component(id=Golgi_Na type=ionChannelHH), from gateHHInstantaneous; null
    ? Path not present in component, using factor: 1
...
PROCEDURE rates() {
    ...
    ? multiply applied to all instances of q10 in: <q10Settings> ([Component(id=null type=q10ExpTemp)]) c2 ([Component(id=null type=HHExpRate), Component(id=null type=HHExpLinearRate), Component(id=null type=q10ExpTemp)])
    ...
    ? multiply applied to all instances of q10 in: <q10Settings> ([Component(id=null type=q10ExpTemp)]) c2 ([Component(id=null type=Golgi_Na_h_beta_rate), Component(id=null type=HHExpRate), Component(id=null type=q10ExpTemp)])
    ...
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=h type=gateHHrates), from reverseRate; Component(id=null type=Golgi_Na_h_beta_rate)
